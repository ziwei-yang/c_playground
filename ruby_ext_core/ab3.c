...
#define _rbstr(t,v) ((t==T_STRING) ? RSTRING_PTR(v) : (t==T_NIL) ? "(N)" : "(?)")
#define _rbdbl(t,v) (t==T_FLOAT ? NUM2DBL(v) : \
			((t==T_FIXNUM || t==T_BIGNUM || t==T_DATA) ? (double)(NUM2LONG(v)) : \
			(t==T_NIL ? -1 : -2)))
#define _rblong(t,v) (t==T_FLOAT ? (long)(NUM2DBL(v)) : \
			((t==T_FIXNUM || t==T_BIGNUM || t==T_DATA) ? NUM2LONG(v) : \
			(t==T_NIL ? -1 : -2)))

#define rbstr(v) _rbstr(TYPE(v), v) // MACRO brings multiple executions
//#define rbdbl(v) _rbdbl(TYPE(v), v) // MACRO brings multiple executions
static double rbdbl(VALUE v) { return _rbdbl(TYPE(v), v); }
//#define rblong(v) _rblong(TYPE(v), v) // MACRO brings multiple executions
static long   rblong(VALUE v) { return _rblong(TYPE(v), v); }
...
/* All defined at Object: Object.STR_sell = "sell" */
...
static VALUE init_str_const(const char *s) {
	// str_const_name: STR_#{s}, const must start with upcase
	char  str_const_name[strlen(s)+10];
	str_const_name[0] = 'S';
	str_const_name[1] = 'T';
	str_const_name[2] = 'R';
	str_const_name[3] = '_';
	strcpy(str_const_name+4, s);
	ID id = rb_intern(str_const_name);
...
void ab3_init() {
	...
	str_sell = init_str_const("sell");
	str_buy = init_str_const("buy");
	str_T = init_str_const("T");
	str_p = init_str_const("p");
	str_s = init_str_const("s");
	str_pair = init_str_const("pair");
	str_market = init_str_const("market");
	str_p_take = init_str_const("p_take");
	str_i = init_str_const("i");
	str_status = init_str_const("status");
	str_executed = init_str_const("executed");
	str_remained = init_str_const("remained");
	str_v = init_str_const("v");
	str_remained_v = init_str_const("remained_v");
...
static void cache_trader_attrs(VALUE self) {
	...
	c_arbitrage_min = rbdbl(rb_ivar_get(self, id_my_arbitrage_min));
	c_last_order_arbitrage_min = rbdbl(rb_ivar_get(self, id_my_last_order_arbitrage_min));
	...
	for (int i=0; i < my_markets_sz; i++) {
		c_min_vols[i] = rbdbl(rb_funcall(cv_mkt_clients[i],
			rb_intern("min_vol"), 1, my_pair));
		c_min_qtys[i] = rbdbl(rb_funcall(cv_mkt_clients[i],
			rb_intern("min_quantity"), 1, my_pair));
		c_qty_steps[i] = rbdbl(rb_funcall(cv_mkt_clients[i],
		...
		c_price_ratio_ranges[i][0] = rbdbl(rbary_el(v_price_ratio_range, 0));
		c_price_ratio_ranges[i][1] = rbdbl(rbary_el(v_price_ratio_range, 1));
		if (c_price_ratio_ranges[i][0] >= c_price_ratio_ranges[i][1]) {
			URN_WARNF("c_price_ratio_ranges %8s %4.4lf %4.4lf", c_mkts[i],
				c_price_ratio_ranges[i][0], c_price_ratio_ranges[i][1]);
			rb_raise(rb_eTypeError, "price_ratio_range should be [min, max]");
		}
		c_p_steps[i] = rbdbl(rb_funcall(cv_mkt_clients[i],
		...
		if (TYPE(_last_t_suggest_new_spike_p) == T_NIL)
			...
			c_my_last_t_suggest_new_spike_prices[i] = rbdbl(_last_t_suggest_new_spike_p);
	...
	c_vol_zero = rbdbl(rb_ivar_get(self, rb_intern("@vol_zero")));
	...
	c_typebc_cap_rate = rbdbl(rb_ivar_get(self, rb_intern("@typeBC_capacity_rate")));
	c_single_vol_ratio = rbdbl(rb_ivar_get(self, rb_intern("@single_vol_ratio")));
...
static void order_init(struct order *o) {
	memset(o, 0, sizeof(struct order));
	o->m[0] = '?';
	o->status[0] = '?';
	o->desc[0] = '?';
	o->i[0] = '?';
...
static struct order *rborder_to_order(
	...
	o->p = rbdbl(rb_hash_aref(v_o, str_p));
	o->s = rbdbl(rb_hash_aref(v_o, str_s));
	o->executed = rbdbl(rb_hash_aref(v_o, str_executed));
	o->remained = rbdbl(rb_hash_aref(v_o, str_remained));
	// _ab3_dbg("\t p %4.8lf s %4.4lf e %4.4lf r %4.4lf", o->p, o->s, o->executed, o->remained);
	if (TYPE(rb_hash_aref(v_o, str_v)) != T_NIL) {
		o->vol_based = true;
		o->v = rblong(rb_hash_aref(v_o, str_v));
		o->executed_v = rblong(rb_hash_aref(v_o, str_executed_v));
		o->remained_v = rblong(rb_hash_aref(v_o, str_remained_v));
	} else
		o->vol_based = false;
	o->ts_e3 = rblong(rb_hash_aref(v_o, str_t));
...
static bool _aggressive_arbitrage_orders_c(
	...
	while (1) {
		...
		_ab3_dbg("loop %d scan %s odbk %d", loop_ct,
			(type == BUY ? "BUY" : "SEL"), idx);

		if ((scan_status[opposite_type][_idx_fin] == 1) &&
				(scan_status[type][_idx_size_sum] >= scan_status[opposite_type][_idx_size_sum])) {
			_ab3_dbg("\toppo %d side finished, "
				"%d side size_sum >= opposite, all finished.",
				opposite_type, type);
		...
		_ab3_dbg("\tODBK %d %d %4.8lf %4.4lf %4.10lf", type, idx, p, s, p_take);
		if ((p <= 0) || (s <= 0) || (p_take <= 0)) {
		...
		_ab3_dbg("\tstatus sz_sum %4.4lf avg_p %4.8lf", size_sum, avg_price);
		...
		if (s + size_sum >= vol_max) {
			...
			scan_status[type][_idx_fin] = 1;
			_ab3_dbg("\tMax vol reached, vol_max %4.4lf", vol_max);
		}
		...
		double opposite_p_take = scan_status[opposite_type][_idx_p_take];
		...
		if (opposite_s <= 0) {
			...
			_ab3_dbg("\tScan %s orderbook %d: O "
				"p %4.8lf p_tk %4.10lf s %4.4lf init this side.",
				(type == BUY ? "BUY" : "SEL"),
				idx, p, p_take, s);
			scan_buy_now = !scan_buy_now;
			continue;
		...
		_ab3_dbg("\tnow %d size_sum:%4.4lf p:%4.8lf avg_p:%4.8lf",
			type, size_sum, p, next_avg_price);
		scan_status[type][_idx_idx] += 1;
	...
	buy_order->s  = size;
	sell_order->s = size;
	_ab3_dbg("buy_order  %s", rborder_desc(buy_order));
	_ab3_dbg("sell_order %s", rborder_desc(sell_order));
	_ab3_dbg("size %4.4lf vol_min %4.8lf", size, vol_min);
	...
	_ab3_dbg("min_bid_sz %4.4lf min_ask_sz %4.8lf", min_bid_sz, min_ask_sz);
...
VALUE _new_ab3_chance(
	...
	double p = rbdbl(rb_hash_aref(mo, str_p));
	double rate = -1;
	...
	if (strcmp("A", cata) == 0) {
		sprintf(explain, "Cata  %4s: %s %8.8lf at %8.8lf %8s, %s at %s",
			...
			rbdbl(rb_hash_aref(mo, str_s)),
			rbdbl(rb_hash_aref(mo, str_p)),
			RSTRING_PTR(v_m),
...
VALUE method_detect_arbitrage_pattern(VALUE self, VALUE v_opt) {
	...
	c_vol_max_const = rbdbl(rb_ivar_get(self, rb_intern("@vol_max_const")));
	c_my_debug = (TYPE(rb_ivar_get(self, id_my_debug)) == T_TRUE) ? true : false;
	...
	double vol_max_a = rbdbl(rbhsh_el(my_vol_max, "A"));
	double vol_max_b = rbdbl(rbhsh_el(my_vol_max, "B"));
	double vol_max_c = rbdbl(rbhsh_el(my_vol_max, "C"));
	double avg_last = rbdbl(my_avg_last);
	...
	for (int m1_idx=0; m1_idx < my_markets_sz; m1_idx++) {
		...
		for (int m2_idx=0; m2_idx < my_markets_sz; m2_idx++) {
			...
			order_init(&buy_order);
			order_init(&sell_order);
			...
			// downgrade_order_size_for_all_market() in C way.
			...
			URN_LOGF("After equalize_order_size(), downgrade_order_size_for_all_market()\n"
				"BUY  p %8.8f s %8.8f %s \n"
				"SELL p %8.8f s %8.8f %s \n",
				rbdbl(rb_hash_aref(v_ob, str_p)), rbdbl(rb_hash_aref(v_ob, str_s)), buy_order.m,
				rbdbl(rb_hash_aref(v_os, str_p)), rbdbl(rb_hash_aref(v_os, str_s)), sell_order.m);
			...
			if (rbdbl(rb_hash_aref(v_os, str_s)) < min_ask_sz) {
				URN_LOGF("\t Sell order break min_order_size %4.4lf", min_ask_sz);
				continue;
			} else if (rbdbl(rb_hash_aref(v_ob, str_s)) < min_bid_sz) {
				URN_LOGF("\t Buy  order break min_order_size %4.4lf", min_bid_sz);
				continue;
			}
			...
			double suggest_size = rbdbl(rb_hash_aref(v_ob, str_s));
	...
	_ab3_dbg("S highest_ask_p %4.8lf lowest_bid_p %4.8lf", highest_ask_p, lowest_bid_p);
	if (c_my_spike_catcher_enabled && (c_run_mode_d || c_run_mode_e)) {
		for (int m1_idx=0; m1_idx < my_markets_sz; m1_idx++) {
			...
			if (bid_p >= 1.01 * lowest_bid_p)
				p_threshold = URN_MIN(p_threshold,
					1 - (1 - lowest_bid_p/bid_p) * 3); // 0.99 -> 0.97, 0.98 -> 0.94
			...
			_ab3_dbg("S spike_buy  %8s spike_bid? %d "
				"f_o_s_avail %4.4lf quota %4.4lf vol_max_cst %4.4f p_thre %lf",
				c_mkts[m1_idx], spike_bid,
				first_order_size_avail, buy_o_quota,
				c_vol_max_const, p_threshold);
			...
			_ab3_dbg("S spike_buy  %8s bid_price[0..3]: %4.8f %4.8f %4.8f %4.8f",
				c_mkts[m1_idx], bid_prices[0], bid_prices[1], bid_prices[2], bid_prices[3]);
			...
			_ab3_dbg("S spike_sell %8s spike_ask? %d "
				...
				first_order_size_avail, sell_o_quota,
				c_vol_max_const, p_threshold);
			...
			// print size details
			_ab3_dbg("S spike_sell %8s ask_price[0..3]: %4.8f %4.8f %4.8f %4.8f",
				c_mkts[m1_idx], ask_prices[0], ask_prices[1], ask_prices[2], ask_prices[3]);
		...
		for(int lp_i=0; lp_i < my_markets_sz; lp_i++) {
			...
			for(int agg=0; agg < MAX_AGRSV_TYPES; agg++) {
				...
				_ab3_dbg("4.1 f_o_s_avail %5.8lf sug_s %5.8lf max_s %5.8lf "
					"s_v_ratio %4.4lf vol_max_cata %5.8lf",
				...
				_ab3_dbgc(RED, "urn_round %4.4lf to %d", suggest_size, c_size_precise);
				suggest_size = urn_round(suggest_size, c_size_precise);
				...
				rb_hash_aset(mo, str_p_real, DBL2NUM(p_real));
